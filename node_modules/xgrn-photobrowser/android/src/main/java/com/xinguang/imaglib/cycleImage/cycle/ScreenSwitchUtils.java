 ...
 * 记得清除掉所有的监听
 *
 * @author wl on 2017/9/4 0004 18:06
 */
...
public class ScreenSwitchUtils {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 888:
                    ...
                    Log.e("ScreenSwitchUtils", "orientation: " + orientation);
     ...
     * 申请监听，当前角度
     */
    public  void addAngleToMonitor(AngleToMonitor angleToMonitor) {
        if (list == null) {
            list = new ArrayList<>();
        }
        list.add(angleToMonitor);
    }

    public interface AngleToMonitor {
        void setAngles(int orientation);
    }

    private static List<AngleToMonitor> list;

    //自己用来设置角度的方法
    private void setAngles(int orientation) {
        if (list == null) return;
        for (AngleToMonitor angleToMonitor : list) {
            if (angleToMonitor == null) return;
            angleToMonitor.setAngles(orientation);
        }
    }
}
