...
#import "XGOAWaitingView.h"
////waitView只是用来显示图片下载进度的，现在用YYImageView 用的类似webImage的解析方式，所以就不用自定义一个view了先隐藏

@interface XGOABrowserImageView ()
//@property (nonatomic, weak) XGOAWaitingView *waitingView;
@property (nonatomic, assign) BOOL didCheckSize;
...
@implementation XGOABrowserImageView
...
//    _waitingView.center = CGPointMake(self.frame.size.width * 0.5, self.frame.size.height * 0.5);
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = XGOAPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
//    XGOAWaitingView *waiting = [[XGOAWaitingView alloc] init];
//    waiting.bounds = CGRectMake(0, 0, 100, 100);
//    waiting.waitMode = XGOAWaitingViewProgressMode;
//    _waitingView = waiting;
//    [self addSubview:waiting];
    
  __weak __typeof(&*self) weakSelf = self;
  [self yy_setImageWithURL:url placeholder:placeholder options:YYWebImageOptionProgressive progress:^(NSInteger receivedSize, NSInteger expectedSize) {
    __strong __typeof(&*weakSelf) strongSelf = weakSelf;
    strongSelf.progress = (CGFloat)receivedSize / expectedSize;
  } transform:nil completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
    __strong __typeof(&*weakSelf) strongSelf = weakSelf;
    [strongSelf removeWaitingView];
    if (error) {
      UILabel *label = [[UILabel alloc] init];
      label.bounds = CGRectMake(0, 0, 160, 30);
      label.center = CGPointMake(strongSelf.bounds.size.width * 0.5, strongSelf.bounds.size.height * 0.5);
      label.text = @"图片加载失败";
      ...
      ///不主动再去设置，用框架默认的设置方法，如果不行再来主动设置
//      strongSelf.scrollImageView.image = image;
//      [strongSelf.scrollImageView setNeedsDisplay];
    }
  }];
}
...
- (void)prepareForImageViewScaling {
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = XGOAPhotoBrowserBackgrounColor;
